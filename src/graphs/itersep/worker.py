...
class GraphIteracionesXEpisodioWorker(QtCore.QObject):
    def __init__(self, input_data):
        super(GraphIteracionesXEpisodioWorker, self).__init__()
    ...
    def _init_plt(self):
        ...
        try:
            ...
            figure = plt.gcf()
            figure.canvas.set_window_title(u"Número de iteraciones por episodio")

            plt.plot(xrange(1, len(y_values) + 1), y_values)
            plt.grid(True)
            plt.title(u"Número de iteraciones por episodio")
            plt.xlabel(u"Episodios")
            plt.ylabel(u"Iteraciones")
